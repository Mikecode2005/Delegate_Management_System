...
def create_circular_progress(value, max_value, title, color):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                {'range': [0, max_value], 'color': f"{border_color}"}
            ],
            'threshold': {
...
if st.session_state.df is not None and not st.session_state.df.empty:
    ...
    st.markdown(f'<div class="modern-card">', unsafe_allow_html=True)
    tabs = st.tabs(["📊 Analysis", "🔍 Search & Filter", "⚙️ Data Processing", "💾 Export"])
    ...
    with tabs[2]:
        st.markdown(f'<div class="section-title">⚙️ Data Processing</div>', unsafe_allow_html=True)
